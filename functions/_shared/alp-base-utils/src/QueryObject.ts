...
export class QueryObject {
  ...
  public executeQuery<T>(
    ...
    const run = (internalCallback: CallBackInterface) => {
      ...
      logger.debug(`[QueryObject.executeQuery]schemaName: ${JSON.stringify(schemaName)}`)
      logger.debug(`[QueryObject.executeQuery]connection: ${JSON.stringify(connection)}`)
  ...
  public executeQueryOnWriteConnection<T>(
    ...
    const run = (internalCallback: CallBackInterface) => {
      ...
      connection.executeUpdate(
        ...
        preparedQuery.placeholders,
        (err, resultData) => {
          if (connection.constructor.name === "TrexConnection" && resultData?.error) {
