...
describe("translateHanaToPostgres where hana and postgres have same syntax", () => {
  it("should ", () => {
    const input = ``;
    const expected = ``;
    expect(translateHanaToPostgres(input, "schema", "vocab_schema", "schema")).toBe(expected);
  });
  ...
  it("should reduce extra whitespace to a single space", () => {
    const input1 = ` `;
    const expected1 = ` `;
    expect(translateHanaToPostgres(input1, "schema", "vocab_schema", "schema")).toBe(expected1);
    const input2 = `   `;
    const expected2 = ` `;
    expect(translateHanaToPostgres(input2, "schema", "vocab_schema", "schema")).toBe(expected2);
  });
