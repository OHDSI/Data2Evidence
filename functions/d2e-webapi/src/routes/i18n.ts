...
import { LocalesResponseDto, I18nLangResponseDto } from "../dto/i18n.ts";

// deno-lint-ignore require-await
export const i18n: FastifyPluginAsyncZod = async function (app) {
  app.get(
    "/",
    {
      schema: {
        description: "Get i18n config for query lang",
        querystring: z.object({ lang: z.string() }),
        response: { 200: I18nLangResponseDto },
      },
    },
    (req, res) => {
      ...
      if (lang === "en") {
        const enLangResponse = {
          ...
          commonErrors: {
            noSources:
              'The current WebAPI has no sources defined.<br/>Please add one or more on <a href="#/configure">configuration</a> page.',
            webapiConnectError:
              "Unable to connect to an instance of the WebAPI.<br/>Please contact your administrator to resolve this issue.",
            browserWarning:
          ...
          options: {
            ...
            exactly: "exactly",
            fixedDurationRelativeToInitialEvent:
            ...
            person: "Person",
            reporting: {
              personsAndExposureDuringBaselinePeriod:
                "Persons and Exposure during baseline period",
              personsAndExposureDuringCohortPeriod:
                "Persons and Exposure during cohort period",
              visitsDuringBaselinePeriod: "Visits during baseline period",
              visitDatesDuringBaselinePeriod:
                "Visit-dates during baseline period",
              careSiteVisitDatesDuringBaselinePeriod:
                "Care-site-visit-dates during baseline period",
              visitsDuringCohortPeriod: "Visits during cohort period",
              visitDatesDuringCohortPeriod: "Visit-dates during cohort period",
              careSiteVisitDatesDuringCohortPeriod:
                "Care-site-visit-dates during cohort period",
              drugUtilizationDuringBaselinePeriod:
                "Drug Utilization during baseline period",
              drugUtilizationDuringCohortPeriod:
                "Drug Utilization during cohort period",
            },
          },
          components: {
            authorship: {
            ...
            analysisExecution: {
              ...
              noCompletedExecutionsForDataSource:
                "There is no completed executions for the data source yet.",
              stopGenerationConfirmation: "Do you want to stop generation?",
            },
            atlasCohortEditor: {
              enterCohortPlaceholder:
            ...
            expressionCartoonBindings: {
              ...
              visitCapital: "Visit",
              expressionCartoonBindingsText_1:
                "the following primary criterion",
              expressionCartoonBindingsText_2:
                "<%=oneOrAny%> of the following <%=pcListLength%> primary criteria",
              expressionCartoonBindingsText_3:
                "Results will be generated for every person event matching <%=pcCritMatch%>. Final results will be limited to <%=thoseOrthe%> events matching any additional criteria and inclusion rules.",
              expressionCartoonBindingsText_4:
                "Results will be generated for the <%=pLimitTypeToLowerCase%> single event matching <%=pcCritMatch%>.",
              expressionCartoonBindingsText_5:
                "Result index date<%=pcPlural%> will be the start date<%=pcPlural%> of the matching primary criteria event<%=pcPlural%>.",
              expressionCartoonBindingsText_6:
                "Restrict to people having events matching <%=critgroupTypeToLowerCase%> of the following criteria.",
              expressionCartoonBindingsText_7:
                "Events must start within bracketed period",
              expressionCartoonBindingsText_8:
                "relative to index date. Lines and arrows represent required duration of these events.",
              expressionCartoonBindingsText_9:
                "with <%=type%> <%=count%> using <%=distinctOrAll%> occurrences",
              expressionCartoonBindingsText_10:
                "occurring between <%=startDays%> days <%=beforeArAfterStart%> and <%=endDays%> days <%=beforeArAfterEnd%> index",
            },
            cohortExpressionEditor: {
              ...
              cohortEntryEventsText_1:
                "Events are recorded time-stamped observations for the persons, such as drug exposures, conditions, procedures, measurements and visits. All events have a start date and end date, though some events may have a start date and end date with the same value (such as procedures or measurements).",
              cohortEntryEventsText_2:
                "Events having any of the following criteria:",
              cohortEntryEventsText_3:
              ...
              cohortExitText_1:
                "For all qualifying events, there must be a specification of when each event ends presence in the cohort. By default, an event ends at the end of the observation period containing the qualifying initial event, but event end dates may be also be specified:",
              cohortExitText_2:
                "The qualifying initial event can be set to end after a fixed time period relative to the event start or end date, or the end date can be based on the end of a continuous exposure to a set of drugs identified by a concept set.",
              cohortExitText_3:
                "Criteria can be used to signal the end of an event. If a censoring event is identified between a qualifying event's start and end date, the date of the censoring event will be used as the exit date.",
            ...
            cohortExpressionViewer: {
              cohortExpressionViewerText_1:
                "People having any of the following:",
              cohortExpressionViewerText_10: "Limit qualifying cohort to:",
              cohortExpressionViewerText_11: "Censoring Events:",
              cohortExpressionViewerText_12:
                "Exit Cohort based on the following:",
              cohortExpressionViewerText_13: "Cohort Collapse Strategy:",
              cohortExpressionViewerText_14:
                "Collapse cohort by era with a gap size of ",
              cohortExpressionViewerText_15:
                "Inclusion Criteria #(<%=index%>): <%=name%>",
              cohortExpressionViewerText_2:
                "with continuous observation of at least",
              ...
              cohortExpressionViewerText_7:
                "For people matching the Primary Events, include:",
              cohortExpressionViewerText_8:
                "Limit cohort of initial events to:",
            ...
            conceptSet: {
              ...
              notEnoughPermissions:
                "Not enough permissions to <%=action%> Concept Set",
              actionEdit: "edit",
              actionCreate: "create",
              overwriteConfirm:
                "Are you sure you want to overwrite current Concept Set Expression?",
              ...
              expressionResolveError:
                "Error occurred during resolving expression!",
              forbiddenError:
                "You are not authorized to view the concept set expression.",
              import: {
                repository: {
                  overwriteCheckboxLabel:
                    "Overwrite concept set items (append if not checked)",
                },
                sourceCodes: {
            ...
            conditionMeasurement: {
              ...
              conditionMeasurementText_19:
                "measurement value operator is any of:",
              conditionMeasurementText_2:
                "for the first time in the person's history",
            ...
            conditionObservationPeriod: {
              ...
              conditionObservationText_2:
                "limited to the patients first observation period",
            ...
            conditionPayerPlanPeriod: {
              ...
              conditionPayerPlanText_1:
                "payer plan periods with the following criteria:",
              ...
              conditionPayerPlanText_18:
                "Note: Payer Plan Period criteria is only available in CDM v5.3 and later.",
              conditionPayerPlanText_19: "a payer plan period",
              conditionPayerPlanText_2:
                "limited to the patients first payer plan period",
            ...
            conditionProcedureOccurrence: {
              ...
              conditionProcedureOccurrenceText_10:
              ...
              conditionProcedureOccurrenceText_13:
                "provider specialty is any of:",
              conditionProcedureOccurrenceText_14:
                "visit occurrence is any of:",
              conditionProcedureOccurrenceText_2:
                "for the first time in the person's history",
              ...
              conditionProcedureOccurrenceText_6:
                "Procedure Source Concept is:",
              conditionProcedureOccurrenceText_7: "with age:",
              conditionProcedureOccurrenceText_8: "with a gender of:",
              conditionProcedureOccurrenceText_9:
            ...
            conditionVisit: {
              ...
              conditionSpecimeText_12:
                "with distance from a Place of Service to Patient",
            ...
            conditionVisitDetail: {
              ...
              conditionVisitDetailText_11:
                "with a Place of Service located in:",
              conditionVisitDetailText_12:
                "with distance from a Place of Service to Patient:",
              conditionVisitDetailText_2:
                "for the first time in the person's history",
            ...
            customEraStrategy: {
              ...
              customEraStrategyText_3:
                "Concept set containing the drug(s) of interest:",
              customEraStrategyText_4:
                "Persistence window: allow for a maximum of ",
              customEraStrategyText_5:
            ...
            cyclops: {
              controlEditor: {
                ...
                crossValidationSearchType:
                  "Select the cross validation search type:",
                ...
                nameOfExchangeableSamplingUnit:
                  "Name of exchangeable sampling unit. Option <i>byPid</i> selects entire strata. Option <i>byRow</i> selects single rows. If set to <i>auto</i>, <i>byRow</i> will be used for all models except conditional models where the average number of rows per stratum is smaller than the number of strata:",
                noiseLevel: "Noise level for Cyclops screen output:",
                numberOfRandomFolds:
                  "Number of random folds to employ in cross validation:",
                numberOfRepetitions:
                  "Number of repetitions of <%=num%>-fold cross validation:",
                ...
                startingVariance:
                  "Starting variance for auto-search cross-validation; -1 = use estimate based on data:",
                minDataCrossValidation:
                  "Minimum amount of data required for cross-validation:",
              ...
              prior: {
                ...
                distributionVariance:
                  "Specify the prior distribution variance:",
                ...
                vector:
                  "Specify a vector of numbers or covariate Ids to exclude from prior:",
              },
            },
            dateOffsetStrategy: {
              dateOffsetStrategyText_6: "Date Offset Exit Criteria",
              dateOffsetStrategyText_7:
                "This cohort definition end date will be the index event's",
              dateOffsetStrategyText_8: "plus",
              dateOffsetStrategyText_1: "Fixed Duration Persistence:",
              dateOffsetStrategyText_2:
            ...
            evidencePairViewer: {
              ...
              evidencePairViewerText_2:
                "If CommonEvidenceModel has indicated there is product label information for the selected drug-condition pair (1 = true, 0 = false) you may see many product labels below. The count of labels is non-informative since an adverse event on one label is supposed to be on all product labels. So, this page will show all product labels for the selected drug-condition pair.",
              evidencePairViewerText_3:
                "If the CommonEvidenceModel has listed a number of publications, on the “Viewing Evidence” pop-up, one publication may occur multiple times in the results below.  This is because for every drug-condition pair a publication matches, the publication will show up in “View Evidence” list.",
              evidencePairViewerText_4:
                "Click here for more information about this evidence list",
              evidencePairViewerText_5: "Hide details",
              evidencePairViewerText_6:
                "The concept set is not marked as valid to generate results. Please make sure this concept set contains only CONDITIONS or DRUGS.",
            },
            executionUtils: {
              startNewExecutionInParallelConfirmation:
                "A generation for the source has already been started. Are you sure you want to start a new one in parallel?",
            },
            exploreCohort: {
              ...
              exploreCohortText_2:
                "This cohort has no members available to view for the selected data source. Please ensure that you have generated the cohort using the Generation tab.",
            },
            export: {
              button: "Export",
            ...
            exportSql: {
              incorrectDesign:
                "Cannot generate SQL: design of the analysis is incorrect",
            },
            ...
            feasibilityReportViewer: {
              ...
              feasibilityReportViewerText_8:
                "No inclusion rules specified for this cohort definition.",
            },
            featureextraction: {
            ...
            period: {
              ...
              periodText_1:
                "Note: only observation periods that encompass the start and end dates will be used.",
              starting: "starting",
            },
            textFilter: {
              contains: "Containing",
            ...
            welcome: {
              ...
              saml: {
                loginRequired:
                  "Please log out from your Identity Provider and try to log in again",
              },
            },
            windowInput: {
            ...
            nameValidation: {
              ...
              invalidCharacters:
                'The name cannot contain any of the following characters: \\/:*?"<>|  Please choose a different name.',
              length:
                "The name length exceeds <%=length%> symbols. Please choose a different name.",
              defaultName:
            ...
            tags: {
              ...
              cannotUnassignProtectedTagWarning:
                "Cannot unassign protected tag: <%=tagName%>",
              reassignConfirm:
                'The maximum number of assigned tags in the tag group "<%=tagGroup%>" is <%=maxNumber%>. The tag "<%=tagName%>" will be unassigned. Proceed?',
            },
            versions: {
            ...
            reusablesModal: {
              title: "Insert from Reusable",
              loadingReusables: "Loading reusables list...",
              anyConceptSet: "Any Concept Set",
            },
            heading: {
              tag: {
                tooltip: {
          ...
          facets: {
            caption: {
              ...
              errorMsg: "Error Msg",
              foundInProductLabelOrPublications:
          ...
          columns: {
            ...
            progress: "Progress",
            provider: "Provider",
            publicationCountAncestor:
              "Publication Count (Ancestor Concept Match)",
            publicationCountDescendant:
            ...
            rc: "RC",
            recommendedRelationshipCount: "Count",
          ...
          dataSources: {
            ...
            deathReport: {
              ...
              deathPrevalenceByAgeGenderYear:
            ...
            observationPeriodReport: {
              ...
              personsWithContinuousObservationByYear:
            ...
            treemap: {
              legend: {
                ...
                color: "Color:",
                colorExplanation:
                  "(Light to Dark = Low to High), Use Ctrl-Click to Zoom, Alt-Click to Reset Zoom",
              },
              treemapTab: "Treemap",
            },
          },
          ...
          cs: {
            browser: {
              ...
              compare: {
                ...
                sameConcepts:
                  "Compared Concept Sets contain Concepts which are identical",
                ...
                saveWarning:
                  "You must save the current concept set before you can perform this comparison.",
                vennDiagram: "Venn Diagram",
                vennDiagramAmountConcepts: "The amount: <%=count%>",
                vennDiagramAmountOnly:
                  "The concepts are only in this concept set: <%=amountOnly%>",
                vennDiagramCommonConcepts: "Common Concepts",
                vennDiagramNameConceptSet: "<%=name%> concept set",
                vennDiagramTotalAmountConcepts:
                  "The total amount of concepts: <%=count%>",
                viewCountMessage:
                  "View database record counts (RC) and descendant record counts (DRC) for:",
              },
              exportConceptSetsButton: "Export Concept Sets",
              exportConceptSetsNumber:
                "<b><%=num%></b> concept sets selected for export.",
            ...
            manager: {
              attemptingToFindMessage:
                "Attempting to find an optimal definition for this concept set...",
              cancelButton: "Cancel",
              caption: "Concept Set #<%=id%>",
              captionVersion:
                "Concept Set #<%=id%> - Version <%=number%> Preview",
              changeColumnsButton: "Change Columns",
              conceptsRemovedHeading: "Concepts Removed",
              conceptsRemovedMessage:
                "The following concepts were removed since the optimized version already includes them.",
              createOption: "Create New Concept Set",
              csAlreadyExistsMessage:
                "A concept set with this name already exists. Please choose a different name.",
              csDeleteConfirmMessage:
                "Delete concept set? Warning: deletion can not be undone!",
              csNotSavedConfirmMessage:
                "Your concept set changes are not saved. Would you like to continue?",
              csSaveErrorMessage:
              ...
              exploreEvidence: {
                ...
                anchor2:
                  "High level information on negative controls processing",
                anchor3: "Additional resources",
                chooseAConceptSet: "Choose a Concept Set",
                deletedCcMessage:
                  "The concept set specified (#<%=cs%>) was deleted. Please select a different concept set.",
                ...
                excludeLabel:
                  "What concepts do you want to exclude as known positive controls? (Optional)",
                generate: "Generate",
                generated: "Generated",
                ...
                includeLabel:
                  "What concepts do you want to include as known negative controls? (Optional)",
                invalidCsWarning:
                  "Your saved concepts come from multiple domains or from a domain outside of conditions or drugs. The concept set must contain ONLY conditions OR drugs in order to explore evidence.",
                loadingEvidence: "loading evidence",
                message1: "Evidence comes from the",
                message2:
                  "The evidence base is comprised of evidence found in published literature, product labels and spontaneous reports. Use the evidence sources below to find out what evidence has been associated to the concepts included in this concept set.",
                noCsWarning:
                  "You must define a concept set with drugs found in the RxNorm vocabulary at the Ingredient class level OR Conditions from SNOMED. The concept set must contain ONLY conditions OR drugs in order to explore evidence.",
                noEvidenceFoundFor: "No evidence found for",
                saveNewButton: "Save New Concept Set From Selection Below",
                title: "Explore Evidence:",
                viewDatabaseRecordCounts:
                  "View database record counts (RC) and descendant record counts (DRC) for:",
                viewEvidence: "View Evidence",
                viewingEvidence: "Viewing Evidence",
              },
              fullyOptimizedMessage:
              ...
              pleaseProvideNameMessage:
                "Please provide a name for your new concept set:",
              pleaseReviewMessage:
              ...
              concept: {
                ...
                tabs: {
                  ...
                  hierarchy: {
                    ...
                    caption: "Hierarchy",
                    noHierarchyFoundMessage:
                      "No hierarchy found for non-standard concepts.",
                    view: "View:",
          ...
          cc: {
            ...
            modals: {
              confirmChanges:
                "Your changes are not saved. Would you like to continue?",
              selectConceptSet: "Select Concept Set...",
            ...
            viewEdit: {
              ...
              captionPreview:
                "Characterization #<%=id%> - Version <%=number%> Preview",
              loadingCohortCharacterization:
                "Loading Cohort Characterization...",
              deleteConfirmation:
                "Delete cohort characterization? Warning: deletion can not be undone!",
              design: {
                description:
              ...
              executions: {
                ...
                prevalenceStatConverter: {
                  ...
                  cohort: "Cohort:",
                  exploringConceptHierarchyFor:
                    "Exploring concept hierarchy for:",
                  ancestor: "Ancestor",
              ...
              results: {
                ...
                threshold: {
                  apply: "Apply",
                  label:
                    "Filter results to covariates with prevalence greater than:",
            ...
            fa: {
              ...
              availableVariablesNote:
                "all variables specified in Cohort Characterization parameters",
              ...
              nameExistsAlert:
                "A feature analysis with this name already exists. Please choose a different name.",
              preset: "Preset",
              ...
              warning: "Warning:",
              warningText:
                "You wouldn't be able to change the type of analysis after save.",
            },
          },
          home: {
            description:
            ...
            releaseNotes: {
              issues:
                "This latest release contains <b><%=issues%></b> feature enhancements and issue resolutions:",
          ...
          ir: {
            ...
            captionVersion:
              "Incidence Rate Analysis #<%=id%> - Version <%=number%> Preview",
            cs: {
            ...
            deleteResultConfirmation:
              "Do you really want to remove result of <%=name%>?",
            editor: {
              ...
              timeAtRiskWarningMessage:
                "Error: Time at risk starts on or after time at risk end, resulting in zero time at risk.  Please ensure the start offset is greater than the end offset.",
            },
            nameConflict:
              "An incidence rate with this name already exists. Please choose a different name.",
            newanalysis: "New Analysis",
            notSavedMessage:
              "Incidence Rate Analysis changes are not saved. Would you like to continue?",
            results: {
              accessDenied: "Access denied",
              ...
              loadingGenerationResultErrorMessage:
                "There was an error while loading generation result reports",
              ...
              rerun: "Rerun",
              running: "Running",
              selectTargetAndOutcomeAlert:
                "You should select at least one target and outcome cohort to generate",
            ...
            savingError:
              "An error occurred while attempting to save an incidence rate.",
            tabs: {
              conceptSets: "Concept Sets",
          ...
          pathways: {
            ...
            manager: {
              ...
              captionVersion:
                "Cohort Pathway #<%=id%> - Version <%=number%> Preview",
              design: {
                analysisSettings: {
                  combinationWindow: {
                    description:
                      "Any dates found within the specified collapse days will be reassigned the earliest date.  Collapsing dates reduces pathway variation, leading to a reduction in 'noise' in the result.",
                    title: "Collapse Days:",
                  },
                  description:
                    "The following set of parameters will be used when performing the pathway analysis.",
                  maximumPathLength: {
                    description:
                      "Maximum number of steps in a given pathway to be included in the sunburst plot.",
                    title: "Maximum path length:",
                  },
                  minimumCellCount: {
                    description:
                      "Minimum number of subjects in the target cohort for a given event in order to be counted in the pathway.",
                    title: "Minimum cell count:",
                  },
                  allowRepeats: {
                    description:
                      "Allow cohort events/combos to appear multiple times in the same pathway.",
                    title: "Allow repeats:",
                  },
                  title: "Analysis Settings",
                },
                description:
                  "<b>Cohort Pathway</b> is defined as the process of generating an aggregated sequence of transitions between the Event Cohorts among those people in the Target Cohorts.",
                eventCohorts: {
                  description:
                ...
                targetCohorts: {
                  description:
                    "Each of the Target Cohorts will be analyized for the pathways through the event cohorts.",
              ...
              executions: {
                ...
                results: {
                  ...
                  tableview: {
                    ...
                    eventCohortCountsByRank:
                      "Table <%=index%>b: Event Cohort Counts by Rank",
                    eventCohortCounts: "Table <%=index%>c: Event Cohort Counts",
                    distinctEventCohorts:
                      "Table <%=index%>d: Distinct Event Cohorts",
                    stepN: "Step <%=i%>",
                    exactly: "Exactly <%=v%>",
                  },
                },
              ...
              messages: {
                alreadyExists:
                  "A cohort pathway with this name already exists. Please choose a different name.",
                beforeClose:
                  "Your changes are not saved. Would you like to continue?",
                deleteConfirmation:
                  "Delete cohort pathway? Warning: deletion can not be undone!",
                saveFailed:
                  "An error occurred while attempting to save a cohort pathway.",
          ...
          ple: {
            analysisExistsAlert:
              "An estimation analysis with this name already exists. Please choose a different name.",
            analysisSaveErrorAlert:
              "An error occurred while attempting to save an estimation analysis.",
            caption: "Comparative Cohort Analysis",
            captionNumber: "Comparative Cohort Analysis #<%=id%>",
            changesNotSavedConfirmation:
              "Estimation Analysis changes are not saved. Would you like to continue?",
            deleteConfirmation:
              "Delete estimation specification? Warning: deletion can not be undone!",
            ...
            spec: {
              ...
              analysisSettingsDescription:
                "Add or update the analysis settings",
              ...
              conceptsToExcludeLabel:
                "Concepts to <b>exclude</b> when constructing the covariates to be used in this study.*",
              conceptsToExcludeNoteAnalysis:
                "* Concepts defined here are combined with those defined in the Comparisons section.",
              conceptsToExcludeNoteComparisons:
                "* Concepts defined here are combined with those defined in the Analysis settings section.",
              conceptsToIncludeLabel:
                "Concepts to <b>include</b> when constructing the covariates to be used in this study. (Leave blank if you want to include every concept).*",
              conceptsToIncludeNoteAnalysis:
                "* Concepts defined here are combined with those defined in the Comparisons section.",
              conceptsToIncludeNoteComparisons:
                "* Concepts defined here are combined with those defined in the Analysis settings section.",
              controlSettings: "Control Settings",
              covariateSelection: "Covariate selection",
              covariateSettings: "Covariate Settings",
              descriptionPlaceholder:
                "enter a description here (1000 characters max)",
              ...
              evaluationSettingsPanel: {
                ...
                defineTimeAtRiskWindowEnd:
                  "Define the time-at-risk window end:",
                defineTimeAtRiskWindowStart:
              ...
              outcomeModelPanel: {
                covariates:
                  "Should the covariates also be included in the outcome model?",
                regression:
                ...
                regularization:
                  "Use regularization when fitting the outcome model? *",
                regularizationNote:
                  "* Regularization will not be applied to the exposure of interest, just to the covariates.",
                statisticalModel:
              ...
              propensityScoreAdjustmentPanel: {
                baseSelection:
                  "What is the base selection of subjects where the strata bounds are to be determined? Strata are defined as equally-sized strata inside this selection.",
                ...
                covariateIdsStratify:
                  "One or more covariate IDs in the cohortMethodData object on which subjects should be also stratified.",
                howManyStrata:
                ...
                lowerAndUpperBound:
                  "Specify the lower and upper bound on the preference score for keeping persons (specify as: min %, max %, eg: 25,75):",
                matchingOrStratification:
              ...
              studyPopulationPanel: {
                ...
                endDate:
                  "Study end date - a calendar date specifying the maximum date that a cohort index can appear (leave blank to use all time). <b>Important</b>: the study end date is also used to truncate risk windows, meaning no outcomes beyond the study end date will be considered.",
                minimumObservations:
                ...
                subjectInMultipleCohorts:
                  "If a subject is in multiple cohorts, should time-at-risk be censored when the new time-at-risk start to prevent overlap?",
              },
              timeAtRisk: "Time At Risk",
              timeAtRiskPanel: {
                daysFrom: "days from",
                defineTheTimeAtRiskWindowEnd:
                  "Define the time-at-risk window end:",
                defineTheTimeAtRiskWindowStart:
                  "Define the time-at-risk window start, relative to target/comparator cohort entry:",
                minimumNumberOfDaysAtRisk:
                  "The minimum number of days at risk?",
              },
              usingCovariates:
                "Using OHDSI covariates for propensity score model.",
              view: "View:",
            ...
            util: {
              downloadNameNote:
                "<b>Please Note:</b> The package name should consist of alphanumeric characters only. No spaces or special characters are permitted.",
              downloadNote:
                "Please provide a name for the study package and click download to obtain the study package in ZIP format.",
              downloadPackage: "Download Study Package",
              downloadProblemsNote:
              ...
              issues:
                "Your study has the following issues that prevent you from reviewing the full specification details and downloading the study package:",
          ...
          reusables: {
            ...
            manager: {
              ...
              messages: {
                alreadyExists:
                  "A reusable with this name already exists. Please choose a different name.",
                beforeClose:
                  "Your changes are not saved. Would you like to continue?",
                deleteConfirmation:
                  "Delete Reusable? Warning: deletion can not be undone!",
                saveFailed:
                  "An error occurred while attempting to save a reusable.",
          ...
          tagging: {
            ...
            multiAssign: {
              ...
              unassignTags: "Unassign tags",
              messageGroupNotMultiple:
              ...
              tabs: {
                ...
                cohortPathways: "Cohort Pathways",
                reusables: "Reusables",
              },
            },
          },
          ...
          configuration: {
            alerts: {
              ...
              failUpdatePrioritySourceDaimon:
            ...
            rolesImport: {
              ...
              emptyMessage:
                "Insert JSON containig roles definitions in the textarea",
              fixJson: "Fix JSON",
            ...
            userImport: {
              ...
              wizard: {
                ...
                help: {
                  adLdapUsername: "AD/LDAP Username",
                  adLdapUsernameText:
                    "This field is mapped from LDAP/AD <b>Display Name</b> field.",
                  atlasLogin: "Atlas Login",
                  atlasLoginText:
                    "This field is mapped directly from LDAP/AD <b>login</b> field and would be used as login for Atlas.",
                },
                import: {
                  buttons: {
                    checkAll: "Check all",
                ...
                roleMapping: {
                  header:
                    "Please, provide LDAP/Active Directory groups mapping to Atlas roles if required.",
                },
                selectSource: {
                  buttons: {
                    details: "Details",
                ...
                provider: {
                  connection: {
                    failed:
                      "Connection failed. Please see server logs for details.",
                  },
                },
            ...
            viewEdit: {
              ...
              krb: {
                ...
                validation: {
                  ...
                  realm:
                    "&quot;KrbRealm&quot; is empty. Please specify it in the following format: &quot;KrbRealm=[value];&quot;",
                },
              },
              password: {
                label: "Password:",
              ...
              source: {
                alerts: {
                  ...
                  sourceSelectedError:
                    "An error occurred while attempting to get a source with id=<%=selectedSourceId%>.",
                },
                confirms: {
                  close:
                    "Source changes are not saved. Would you like to continue?",
            ...
            tagManagement: {
              ...
              allowCustom: "Free&#8209;form",
              allowCustomCheckbox: "Allow custom tags creation in this group",
              apply: "Submit",
              color: "Color",
              ...
              exampleTagGroup: "Example",
              exampleTagGroupTag: "Example name",
              exampleTag: "Example",
              icon: "Icon",
              mandatory: "Mandatory",
              multiSelectionCheckbox:
                "Allow multiple tags from the group in a single asset",
              showGroupCheckbox: "Show Tag Group column in assets table",
              showInAssetsBrowser: "Show&nbsp;Column",
              tagsInGroup: "Tags in Group '<%=group%>'",
              tagGroups: "Tag Groups",
              tagGroupColor: "Tag group color",
              tagGroupColorPlaceholder:
                "Enter tag group color in #xxxxxx format",
              tagGroupDescription: "Tag group description",
              tagGroupDescriptionPlaceholder: "Enter tag group description",
              tagGroupIcon: "Tag group icon",
              tagGroupIconPlaceholder:
                "Enter tag group icon as font-awesome class",
              tagGroupName: "* Tag group name",
              tagGroupNamePlaceholder: "Enter tag group name",
              tagGroupNameEmpty: "Tag group name cannot be empty",
              hideTags: "Hide tags",
              showTags: "Show tags",
              edit: "Edit",
              remove: "Remove",
              overrideTagColor: "Override tag group color",
              overrideTagIcon: "Override tag group icon",
              protected: "Protected",
              nameExistsWarning:
                "Tag or Group name '<%=tagName%>' is already in use.",
              errorGroupNotEmpty:
                "Cannot delete tag group: the group contains tags.",
              deleteGroupConfirmation:
                "Deletion cannot be undone! Are you sure?",
              deleteTagConfirmation:
                "If the tag is assigned to an asset, it will be unassigned. Deletion cannot be undone. Are you sure you want to delete the tag?",
            },
          },
          feedback: {
            pleaseContact:
              "For access or questions concerning the Atlas application please contact:",
            title: "Feedback",
          },
          licenseAgreement: {
            accept: "Accept",
            ...
            reject: "Reject",
            rejectWarning:
              "Without accepting this terms & conditions you can't use Atlas",
            title: "License Agreement",
          },
          cohortDefinitions: {
            closeYourCurrentCohort:
              "Please close your current cohort definition before creating a new one",
            cohort: {
              modals: {
                analysisTypes: {
                  ...
                  text_1:
                    "Runs minimal analysis to provide a quick overview of this cohort's year of birth, demographics, drug eras and condition eras. (estimated: 10 minutes)",
                  text_2:
                    "Runs analyses to provide Exposure, Visit and Drug Utilization reports. (estimated: 10 - 40 minutes)",
                  text_3:
                    "Runs a complete set of characterization reports. (estimated: 60 minutes)",
            ...
            cohortDefinitionManager: {
              ...
              cohortDefinitionManagerText_11:
                "The reporting system has encountered an error. Please provide the following details to your ATLAS administration team or submit an issue on ",
              cohortDefinitionManagerText_12: "Create Reporting Job Failed",
              cohortDefinitionManagerText_13: "Please Wait",
              cohortDefinitionManagerText_14:
                "Please wait while for the report status to be loaded.",
              cohortDefinitionManagerText_2:
              ...
              generateQuickAnalysis: {
                descr:
                  "minimal analyses set to provide a quick overview of the cohort",
                duration: "10 minutes",
              },
              ...
              samples: {
                ...
                cohortShouldBeGenerated:
                  "Cohort should be generated before creating samples",
                errorFetchingList:
                  "Error when fetching sample list, please try again later",
                errorDeleting:
                  "Error when deleting sample, please try again later",
                errorFetchingData:
                  "Error when fetching sample data, please try again later",
                errorRefreshing:
                  "Error when refreshing sample, please try again later",
                modal: {
                  ...
                  numberOfPatientsEmpty:
                    "Number of patients must be a positive integer between 1 and 500",
                  ...
                  indicateGender:
                    "Indicate gender (leave empty to select all gender)",
                  male: "Male",
                  female: "Female",
                  other: "Other",
                  submit: "Submit",
            ...
            cohortCaptionPreview:
              "Cohort #<%=id%> - Version <%=number%> Preview",
            cohortreports: {
            ...
            costUtilization: {
              ...
              reportManager: {
                ...
                reportManagerText_17:
                  "Number of Persons by Duration from Cohort Start to Cohort End",
                reportManagerText_18:
          ...
          const: {
            ...
            eventsList: {
              ...
              addFirstDiagnosis: {
                ...
                title_visitDetail: "Add First Visit detail Criteria",
                desc_visitDetail: "Limit Visit detail to the first visit.",
              },
              addAgeAtStart: {
              ...
              addGender: {
                ...
                title_visitDetail: "Add Gender Criteria",
                desc_visitDetail: "Filter Visit detail based on Gender.",
              ...
              addStartDate: {
                ...
                title_visitDetail: "Add Start Date Criteria",
                desc_visitDetail: "Filter Visit detail by the Start Date.",
              ...
              addEndDate: {
                ...
                title_visitDetail: "Add End Date Criteria",
                desc_visitDetail: "Filter Visit detail by the End Date.",
              ...
              addLength: {
                ...
                title_visitDetail: "Add Visit detail Length Criteria",
                desc_visitDetail: "Filter Visit detail by duration.",
              },
              addNested: {
                ...
                desc_drug:
                  "Apply criteria using the drug era as the index event.",
                ...
                title_visitDetail: "Add Nested Criteria...",
                desc_visitDetail:
              ...
              addAge: {
                ...
                desc_deviceexposure:
                  "Filter Device Exposures by age at occurrence.",
                ...
                title_visitDetail: "Add Age at Visit detail Criteria",
                desc_visitDetail: "Filter Visit detail by age at occurrence.",
              ...
              addType: {
                ...
                desc_deviceexposure:
                  "Filter Device Exposures by the Exposure Type.",
                ...
                title_visitDetail: "Add Visit detail Type Criteria",
                desc_visitDetail: "Filter Visit detail by the Type.",
              },
              addStopReason: {
              ...
              addSourceConcept: {
                ...
                title_procedureoccurrence:
                  "Add Procedure Source Concept Criteria",
                desc_procedureoccurrence:
                ...
                title_visitDetail: "Add Visit detail Source Concept Criteria",
                desc_visitDetail:
              ...
              addProviderSpecialty: {
                ...
                title_visitDetail: "Add Provider Specialty Criteria",
                desc_visitDetail:
              ...
              addUniqueId: {
                ...
                desc_deviceexposure:
                  "Filter Device Exposures by Device Unique Id.",
              },
              addQuantity: {
              ...
              addUserDefined: {
                ...
                desc_payerplan:
                  "Specify start and end dates to use for Payer Plan Period.",
              },
              addModifier: {
              ...
              addPlaceServiceLocation: {
                ...
                title_visitDetail: "Add Place of Service Location Criteria",
                desc_visitDetail:
          ...
          notifications: {
            ...
            endTime: "End time",
            explanation:
              "Notifications for jobs will be displayed here. A job could include cohort generation, cohort reporting, or other tasks that can take a long time to complete. This top right notification icon will highlight <i class='fa fa-bell pending'></i> if you have new notifications available.",
          ...
          predictions: {
            analysisSettings: "Analysis Settings",
            confirmCatchSave:
              "An error occurred while attempting to save a prediction analysis.",
            confirmChanges:
              "Patient level prediction changes are not saved. Would you like to continue?",
            confirmDelete:
              "Delete patient level prediction specification? Warning: deletion can not be undone!",
            confirmSave:
              "A prediction analysis with this name already exists. Please choose a different name.",
            ...
            utilitiesText_6:
              "Please provide a name for the study package and click &quot;download&quot; to obtain the study package in ZIP format.",
            utilitiesText_7:
              "The package name should consist of alphanumeric characters only. No spaces or special characters are permitted.",
            utilitiesText_8:
              "Please see the downloads area to review problems with your analysis specification. These must be corrected before you can export your definition.",
            packageName: "Package name...",
          },
        };
        res.send(enLangResponse);
      } else {
  ...
  app.get(
    ...
    {
      schema: {
        description: "Get i18n locales",
        response: { 200: LocalesResponseDto },
      },
    },
    (_req, res) => {
      // TODO: Update logic if required
