  ...
  patient: {
    ...
    attributes: {
      ...
      patientname: {
        ...
        type: "text",
        expression: "CASE WHEN array_length(CAST(@PATIENT.content->'$.name[*].family' AS varchar[]), 1) > 0 THEN replace(list_reduce(CAST(@PATIENT.content->'$.name[*].family' AS varchar[]), (acc, e) -> concat(acc, ' ', e)), '\"', '') ELSE '' END",
        order: 7,
        domainFilter: "",
        standardConceptCodeFilter: "",
      },
      ...
      givenname: {
        ...
        type: "text",
        expression: "CASE WHEN array_length(CAST(@PATIENT.content->'$.name[*].given[*]' AS varchar[]), 1) > 0 THEN replace(list_reduce(CAST(@PATIENT.content->'$.name[*].given[*]' AS varchar[]), (acc, e) -> concat(acc, ' ', e)), '\"', '') ELSE '' END",
        order: 8,
        domainFilter: "",
        standardConceptCodeFilter: "",
      },
