...
@Service()
export class ETLMappingContributorService implements IRoleService {
  private static ROLE = ROLES.ETL_MAPPING_CONTRIBUTOR
  ...
  async getUsers(): Promise<User[]> {
    const subquery = this.db('user_group')
      ...
      .where('b2c_group.role', ETLMappingContributorService.ROLE)
  ...
  async getGroup() {
    return await this.groupService.getGroupByRole(ETLMappingContributorService.ROLE)
  }
  ...
  async isGranted(userId: string): Promise<boolean> {
    ...
    return groups.some(g => g.role === ETLMappingContributorService.ROLE)
  }

  async registerUser(userId: string) {
    const group = await this.groupService.getGroupByRole(ETLMappingContributorService.ROLE)
    if (!group?.id) {
      this.logger.warn(`Unable to find group for role ${ETLMappingContributorService.ROLE}`)
      throw new Error(`Unable to find group for role ${ETLMappingContributorService.ROLE}`)
    }

    this.logger.info(`Register ${ETLMappingContributorService.ROLE} for ${userId}`)
  ...
  async withdrawUser(userId: string) {
    ...
    this.logger.info(`Withdraw ${ETLMappingContributorService.ROLE} for ${userId}`)
