...
export class StudyResultService implements IRoleService {
  ...
  async getUsers(): Promise<User[]> {
    const subquery = this.db('user_group')
      ...
      .where('b2c_group.role', StudyResultService.ROLE)
  ...
  async getGroup() {
    return await this.groupService.getGroupByRole(StudyResultService.ROLE)
  }
  ...
  async isGranted(userId: string): Promise<boolean> {
    ...
    return groups.some(g => g.role === StudyResultService.ROLE)
  }

  async registerUser(userId: string) {
    const group = await this.groupService.getGroupByRole(StudyResultService.ROLE)
    if (!group?.id) {
      this.logger.warn(`Unable to find group for role ${StudyResultService.ROLE}`)
      throw new Error(`Unable to find group for role ${StudyResultService.ROLE}`)
    }

    this.logger.info(`Register ${StudyResultService.ROLE} for ${userId}`)
  ...
  async withdrawUser(userId: string) {
    ...
    this.logger.info(`Withdraw ${StudyResultService.ROLE} for ${userId}`)
