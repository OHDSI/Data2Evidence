...
@Service()
export class JobRunnerService implements IRoleService {
  private static ROLE = ROLES.STUDY_WRITE_DQD_RESEARCHER
  ...
  async getUsers(): Promise<User[]> {
    const subquery = this.db('user_group')
      ...
      .where('b2c_group.role', JobRunnerService.ROLE)
  ...
  async getGroup() {
    return await this.groupService.getGroupByRole(JobRunnerService.ROLE)
  }
  ...
  async isGranted(userId: string): Promise<boolean> {
    ...
    return groups.some(g => g.role === JobRunnerService.ROLE)
  }

  async registerUser(userId: string) {
    const group = await this.groupService.getGroupByRole(JobRunnerService.ROLE)
    if (!group?.id) {
      this.logger.warn(`Unable to find group for role ${JobRunnerService.ROLE}`)
      throw new Error(`Unable to find group for role ${JobRunnerService.ROLE}`)
    }

    this.logger.info(`Register ${JobRunnerService.ROLE} for ${userId}`)
  ...
  async withdrawUser(userId: string) {
    ...
    this.logger.info(`Withdraw ${JobRunnerService.ROLE} for ${userId}`)
