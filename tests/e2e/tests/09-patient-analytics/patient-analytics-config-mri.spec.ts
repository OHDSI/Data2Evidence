...
test(TEST_NAME, async ({ page }) => {
  ...
  await expect(page.getByTestId('pa-pane-left')).toMatchAriaSnapshot(`
    - text: "Create Cohort:"
    - button "D2E"
    - button "Atlas"
    - button "Import"
  ...
  await expect(page.getByTestId('pa-app-container')).toMatchAriaSnapshot(
    `- text: "A filter card has been added: Condition Occurrence A"`
  )
  await expect(page.getByTestId('pa-pane-left')).toContainText('Condition Occurrence')
  ...
  await expect(page.getByTestId('pa-app-container')).toMatchAriaSnapshot(`- text: Filters saved.`)
  await page.getByTestId('pa-pane-left').getByRole('link', { name: 'Cohorts' }).click()
  await expect(page.getByTestId('pa-pane-left')).toContainText('Cohort Test')
  ...
  await expect(page.getByTestId('pa-app-container')).toMatchAriaSnapshot(`- text: Saved filter deleted.`)
  await page.getByTestId('pa-pane-left').getByRole('link', { name: 'Cohorts' }).click()
})
