...
const TEST_NAME = 'patient_analytics_smoketest_mri'
...
//   })
  ...
  await test.step('Add another filter card for Condition Occurrence', async () => {
      await page.getByTitle('Add Filter Card').getByRole('button').click();
      await page.getByRole('menuitem', { name: 'Condition Occurrence' }).click();
      await page.evaluate(() => window.scrollTo(0, 10000)); // Scroll down to make the new card visible
      ...
      try {
        ...
        await expect(page.locator('[id="patient\\.interactions\\.conditionoccurrence\\.2"]').getByText('Viral sinusitis')).toBeVisible({ timeout: 10000 });
        await page.locator('[id="patient\\.interactions\\.conditionoccurrence\\.2"]').getByText('Viral sinusitis -').click({ timeout: 10000 });
        await expect(page.locator('.loading-animation-component')).not.toBeVisible({timeout: 20000})
      } catch (e) {
        //await page.getByRole('button', { name: '+' }).click();
  ...
  await test.step('Add advanced Time filter', async () => {
    ...
    await expect(page.locator('.loading-animation-component')).not.toBeVisible()
    await expect(page.getByText('605 / 2694')).toBeVisible()
  })
  ...
  await test.step('Remove the Condition Occurrence B filter card', async () => {
    await page.getByRole('tab', { name: ' Condition Occurrence B ' }).locator('button').last().click()
  ...
  await test.step('Reset the Condition Occurrence A filter card', async () => {
    await page.getByRole('tab', { name: ' Condition Occurrence A ' }).locator('button').last().click()
    await page.getByRole('menuitem', { name: 'Reset Filter Card' }).click()
    await expect(page.locator('.loading-animation-component')).not.toBeVisible();
  ...
  await expect(page.getByText('Filters_cohort_1')).not.toBeVisible()
});
