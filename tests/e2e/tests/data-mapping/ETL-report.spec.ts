...
test('test', async ({ page }) => {
  ...
  await page.setInputFiles('#upload-csv', path.join(__dirname, 'broadsea_person_200.csv'))
  ...
  await page.waitForTimeout(1000) // Wait for the button to be enabled
  await page.getByRole('button', { name: 'Scan tables' }).dispatchEvent('click')
  const checkbox = page.getByRole('checkbox', { name: 'broadsea_person_200.csv' })
  ...
  await page.waitForTimeout(10000) // Wait for the dialog to close
