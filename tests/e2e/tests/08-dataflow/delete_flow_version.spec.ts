...
test('Delete specific version of ui-flow', async ({ page }) => {
  ...
  await page
    .locator('div.alp-button__container')
    .filter({ hasText: new RegExp('^Create your first dataflow$') })
    .click()
  await page.getByRole('textbox', { name: 'Name' }).fill('testflow')
  await page.getByRole('button', { name: 'Create' }).click()

  await page
    .locator('div.node-type-selection__title')
    .filter({ hasText: new RegExp('^Python$') })
    .click()
