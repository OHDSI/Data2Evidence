...
const TEST_NAME = 'dataset-new-schema-hana-load-plugin-omop53'
...
test(TEST_NAME, async ({ page }) => {
    ...
    await page.getByRole('option', { name: 'omop5-3 [hana_load_plugin]' }).click();
    ...
    await page.getByRole('textbox', { name: 'Token dataset code' }).fill('tshanaloadomop53');
    await page.getByRole('button', { name: 'Add', exact: true }).click();
    await expect(page.getByText('Test Study')).toBeVisible();
    await page.getByRole('link', { name: 'Jobs' }).click();
    // Get the first (top) entry link
    const firstEntry = page.locator('a:has(span:text("datamodel-create-cdm_tshanaloadomop53_"))').first();
