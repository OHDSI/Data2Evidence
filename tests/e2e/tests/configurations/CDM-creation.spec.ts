...
test('test', async ({ page }) => {
  ...
  await test.step('Create a new CDM configuration', async () => {
    await page
      ...
      .getByTestId('button')
      .click()
    const TestConfig = page.locator('ul > li').filter({ hasText: 'TestConfig101' })
    try {
      await expect(TestConfig).toBeVisible({ timeout: 3000 })
    ...
    if ((await page.getByRole('button', { name: 'Create', exact: true }).isVisible()) === false) {
  ...
  await test.step('Placeholder mapping', async () => {
    ...
    // await page.screenshot({ path: path.join(__dirname, 'Placeholder_mapping.png') })
  ...
  //   await expect(page.locator('[id*="dataModelVersionCombo-arrow"]')).toBeVisible()
