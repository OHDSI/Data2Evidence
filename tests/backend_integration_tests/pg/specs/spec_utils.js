 ...
 *
 * @param {Client} pgClient - PG DB client
 * @param {String} schemaName - schema inwhich to truncate
 * @param {Function} cb - callback
 */
function truncateSchema(pgClient, schemaName, cb) {
  getAllTableNamesInSchema(pgClient, schemaName, function (err, tableNames) {
    if (err) {
 ...
 * @param {Function} cb - callback
 */
function truncatePatientData(pgClient, schemaName, cb) {
...
function getAllTableNamesInSchema(pgClient, schemaName, cb, tablePrefixArray) {
  ...
  pgClient.query(sqlCommand, collectTableNames)
}
...
*
* @param {Client} pgClient - HBD DB client
* @param {String} schemaName - schema inwhich to truncate
* @param {String} orgId - name of the organisation
* @param {Function} cb - callback
*/
function createOrg(pgClient, schemaName, orgId, cb) {
  function createOrgItself(callback) {
    ...
    pgClient.query(sqlCommand, callback)
  }
 ...
 * @param {Function} cb - callback
 */
function assignUserToOrg(pgClient, schemaName, user, orgId, cb) {
...
function persistPatientSet(loadedPatientBuilder, hostConfig, configName, finalCallback) {
  ...
  var setupPatientCreatorTask = function (callback) {
    ...
    patientCreator = new PatientCreator(hostConfig.getTestSchemaName(), patientCreationPGClient, createConfig)
  ...
  var stopPGClient = function (err, result) {
    patientCreationPGClient.end()
    var patientIDs
    if (!err) {
  ...
  // Do the actual work here
  async.series([setupPatientCreatorTask, writeTestPatientsTask], stopPGClient)
}
...
function setupFullSystem(params, callback) {
  var connectClientTask = function (cb) {
    ...
    params.pgClient.connect(cb)
  }

  var truncateDbTask = function (cb) {
    params.logger('Truncating test patient data')
    truncateSchema(params.pgClient, params.hostConfig.getTestSchemaName(), cb)
  }
...
function teardownFullSystem(params, callback) {
  var truncateSchemaTask = function (cb) {
    params.logger('Truncating test patient data')
    truncateSchema(params.pgClient, params.hostConfig.getTestSchemaName(), function (err) {
      params.pgClient.end()
      cb(err)
    })
  }
