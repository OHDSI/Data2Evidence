...
function logToConsole(msg) {
  // console.log('In pg_environment: ' + msg);
}
...
*
* @constructor
* @param {Client} pgClient - PG client for connecting to HANA
* @param {string}
...
(default: false)
*/
function TestEnvironment(pgClient, schemaName, randomizeSchemaName) {
 ...
 *
 * Connects the PG client then sets up the specified test schema.
 *
...
TestEnvironment.prototype.callSqlProcedure = function (sqlProc, params, cb) {
  ...
  console.log(`params:\n${params}`)
  try {
    this.pgClient.on('error', (err, client) => {
      if (err) {
