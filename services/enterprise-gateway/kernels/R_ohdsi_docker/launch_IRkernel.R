...
if (!is.null(response_address) && str_length(response_address) > 0){
    ...
    svr_listener_cmd <- stringr::str_interp(gsub("\n[:space]*" , "",
        paste(python_cmd,"-c \"import os, sys, importlib.util;
        file_path = '${listener_file}';
        print('[INFO] === Loading server_listener.py module from ' + file_path + ' ===');
        module_name = 'setup_server_listener';
        
        spec = importlib.util.spec_from_file_location(module_name, file_path);
        gl = importlib.util.module_from_spec(spec);
        sys.modules[module_name] = gl;
