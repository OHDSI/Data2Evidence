...
export const env = {
  ...
  _FORCE_CREATE: _env.WATCH_FUNCTIONS
    ? JSON.parse(_env.WATCH_FUNCTIONS)
    : false,
  ...
  LOGTO__CLIENTID_PASSWORD__BASIC_AUTH:
    _env.LOGTO__CLIENTID_PASSWORD__BASIC_AUTH,
  LOGTO__DEFAULT_TENANT__FQDN_URL:
    _env.LOGTO__DEFAULT_TENANT__FQDN_URL || "https://default.logto.app/api",
  ...
  PREFECT_DOCKER_NETWORK:
    _env.PREFECT_DOCKER_NETWORK || `${_env.PROJECT_NAME}_data`,
  ...
  PREFECT_DOCKER_VOLUMES: _env.PREFECT_DOCKER_VOLUMES
    ? JSON.parse(_env.PREFECT_DOCKER_VOLUMES)
    : [],
  ...
  PLUGINS_SEED_UPDATE: _env.PLUGINS_SEED_UPDATE
    ? JSON.parse(_env.PLUGINS_SEED_UPDATE) === true
    : false,
  PLUGINS_PULL_POLICY: _env.PLUGINS_PULL_POLICY || "IfNotPresent",
  PLUGINS_IMAGE_TAG: _env.PLUGINS_IMAGE_TAG || "develop",
  PLUGINS_FLOW_CUSTOM_REPO_IMAGE_CONFIG:
    _env.PLUGINS_FLOW_CUSTOM_REPO_IMAGE_CONFIG
      ? JSON.parse(_env.PLUGINS_FLOW_CUSTOM_REPO_IMAGE_CONFIG)
      : {},
  PLUGINS_INFORMATION_URL:
    _env.PLUGINS_INFORMATION_URL ||
    "https://feeds.dev.azure.com/data2evidence/d2e/_apis/packaging/Feeds/d2e/packages?api-version=7.1&includeDescription=true",
