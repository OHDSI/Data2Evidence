...
function substituteEnvVars(input: string): string {
  ...
  if (iteration >= maxIterations) {
    console.warn(
      `Warning: Maximum iterations (${maxIterations}) reached for variable substitution. Possible circular reference in: ${input}`
    );
  }
  return result;
}
...
function substituteVariable(varExpression: string): string {
  ...
  if (operatorMatch) {
    ...
    if (envValue == undefined)
      console.log(`### ENV UNDEFINED ${varExpression}`);
...
async function _callInit(
  ...
  const TREX_CURRENT_USER_FUNCTION_NAME =
    getFullyQualifiedUserFunctionName(fnEnv);
  ...
  const options: any = {
    ...
    envVars: _myenv,
    forceCreate: env._FORCE_CREATE || watch,
...
function _addFunction(
  ...
  app.all(url + "/*", authn, authz, (c: Context) =>
    _callWorker(c.req.raw, `${path}`, imports, fncfg, dir, xenv)
  );
}
...
function _addService(app: Hono, url: string, service: string, rmsrc: boolean) {
  ...
  app.all(url + postfix, authn, authz, async (c: Context) => {
    ...
    const path = rmsrc
      ? c.req.raw.url.replace(/^[^#]*?:\/\/.*?\//, "/").replace(url, "")
      : c.req.raw.url.replace(/^[^#]*?:\/\/.*?\//, "/");
    let req = {
      headers: newHeaders,
      method: c.req.method,
      body: c.req.raw.body,
...
export async function addPlugin(
    ...
    for (const r of value.init) {
      if (r.function) {
        ...
        _addInit(
          ...
          r.env,
          xenv,
          r.eszip ? r.eszip : null,
          dir,
          r.waitfor ??
            (r.waitforEnvVar
              ? env[r.waitforEnvVar] ?? Deno.env.get(r.waitforEnvVar)
              : "")
        ); //Object.keys(envVarsObj).map((k) => [k, envVarsObj[k]])
        if (r.delay)
  ...
  if (value.roles) {
    for (const [name, cfg] of Object.entries(value.roles)) {
      let _name;
      if (name === "IDP_ALP_SVC_CLIENT_ID") _name = env.IDP_ALP_SVC_CLIENT_ID;
      else if (name === "IDP_ALP_DATA_CLIENT_ID")
        _name = env.IDP_DATA_SVC_CLIENT_ID;
      else _name = name;
      if (global.ROLE_SCOPES[_name])
        global.ROLE_SCOPES[_name] = global.ROLE_SCOPES[_name]
          .concat(cfg)
          .filter((v: any, i: any, self: any) => self.lastIndexOf(v) == i);
      else global.ROLE_SCOPES[_name] = cfg;
    }
  }
  if (value.scopes) {
