...
export async function initTrex() {
  ...
  try {
    ...
    await fhirConn.execute(
      `SELECT trex_fhir_start('0.0.0.0', ${fhirPort})`,
      [],
    );
  ...
  try {
    ...
    for (const row of credentials) {
      const adminCred = (row.credentials ?? []).find(
        (c: any) => c.userScope === "Admin",
      );
      if (!adminCred) {
        logger.log(
          `[attach-startup] no Admin credential for ${row.id} — skipping __srcdb attach`,
        );
        continue;
      }
      connections.push({
    ...
    for (const c of connections) {
      try {
        ...
        logger.log(
          `[attach-startup] connection ${c.id} attach failed: ${(e as Error).message}`,
        );
      }
    }
    for (const cid of cacheIds) {
      try {
        ...
        logger.log(
          `[attach-startup] cache ${cid} attach failed: ${(e as Error).message}`,
        );
      }
    }
    logger.log(
