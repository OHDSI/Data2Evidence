...
export function addRoutes(app: Hono) {
  ...
  app.get('/portal/env.js', (c: Context) => {
    const clientEnv = {
      ...
      REACT_APP_IDP_REQUIRED_CLAIM: env.IDP_REQUIRED_CLAIM,
      ...
      REACT_APP_IDP_OIDC_CONFIG: `{ "client_id": "${CLIENT_ID}", "redirect_uri": "{window.location.origin}/d2e/portal/login-callback", "authority": "${GATEWAY_PROTOCOL_FQDN}", "authority_configuration": { "issuer": "${env.LOGTO_ISSUER}", "authorization_endpoint": "${AUTHORIZATION_URL}", "token_endpoint": "https://${GATEWAY_WO_PROTOCOL_FQDN}/d2e/oauth/token", "end_session_endpoint": "${END_SESSION_URL}" }, "scope": "${SCOPE}", "refresh_time_before_tokens_expiration_in_second": 180 }`,
