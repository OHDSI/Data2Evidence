...
export function addRoutes(app: Hono) {
  ...
  app.get('/portal/env.js', (c: Context) => {
    const clientEnv = {
      ...
      REACT_APP_IDP_OIDC_CONFIG: `{ "client_id": "${CLIENT_ID}", "redirect_uri": "{window.location.origin}/portal/login-callback", "authority": "${GATEWAY_PROTOCOL_FQDN}", "authority_configuration": { "issuer": "${GATEWAY_PROTOCOL_FQDN}oidc", "authorization_endpoint": "${AUTHORIZATION_URL}", "token_endpoint": "https://${GATEWAY_WO_PROTOCOL_FQDN}/oauth/token", "userinfo_endpoint": "https://${GATEWAY_WO_PROTOCOL_FQDN}/oidc/me", "end_session_endpoint": "${END_SESSION_URL}", "revocation_endpoint": "https://${GATEWAY_WO_PROTOCOL_FQDN}/oidc/token/revocation", "check_session_iframe": null }, "scope": "${SCOPE}", "refresh_time_before_tokens_expiration_in_second": 180, "service_worker_only": false }`,
