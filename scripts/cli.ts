...
class D2ECli {
  ...
  // generator that produced the embedded files, which only d2e has.
  private from_checkout(): boolean {
    return (
  ...
  extract_compose_file(): void {
    // Nothing is staged from a checkout: compose_dir is the repo root there, so
    // every destination is the git-tracked source itself. The embed is only
    // regenerated by `npm run build:ts`, which `npm run local` does not run, so
    // writing it would revert local edits to docker-compose.yml and friends.
    if (this.from_checkout()) {
      return;
    }
  ...
  //
  // Never called from a checkout (see extract_compose_file): there `dir` is
  // services/atlas-db-init itself, and pruning against a lagging embed would
  // delete a script added but not yet re-embedded.
