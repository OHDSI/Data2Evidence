...
def dataflow_ui_plugin(json_graph, import_libs, variables, options, databases=None):
    ...
    n = execute_nodes_flow_wo(generated_nodes, sorted_nodes, variables, import_libs, testmode, databases or [])  # flow
...
def execute_subflow_cluster(node_graph, input, shared_variables, importlibs, test, databases=None):
    ...
    def execute_local_cluster(node_graph, input, shared_variables, importlibs, test, databases=None):
        return submit_tasks_to_runner(node_graph, input, shared_variables, importlibs, test, databases)

    @flow(task_runner=DaskTaskRunner(address=scheduler_address), log_prints=True)
    def execute_kube_cluster(node_graph, input, shared_variables, importlibs, test, databases=None):
        return submit_tasks_to_runner(node_graph, input, shared_variables, importlibs, test, databases)

    @flow(task_runner=DaskTaskRunner(address=scheduler_address), log_prints=True)
    def execute_mpi_cluster(node_graph, input, shared_variables, importlibs, test, databases=None):
        return submit_tasks_to_runner(node_graph, input, shared_variables, importlibs, test, databases)

    def submit_tasks_to_runner(node_graph, input, shared_variables, importlibs, test, databases=None):
        ...
        for nodename in node_graph["nodeobj"].sorted_nodes:
            ...
            subflow_results[nodename] = node_task_execution_wo.submit(
                nodename, node["type"], node["nodeobj"], _input, shared_variables, importlibs, test, databases or []).result()  # Result Obj
            count += 1
    ...
    return subflow_execution_wo(node_graph, input, shared_variables, importlibs, test, databases)


@flow(name="execute-nodes",
      flow_run_name="execute-nodes-flowrun",
      log_prints=True)
def execute_nodes_flow(graph, sorted_nodes, shared_variables, importlibs, test, databases=None):
    nodes = {}
    try:
            ...
            if node["type"] not in [node_type.value for node_type in NodeType]:
                ...
                if node["type"] == NodeType.SUBFLOW:
                    ...
                    result_of_subflow = execute_subflow_cluster(
                        node, _input, shared_variables, importlibs, test, databases or [])
                    ...
                    nodes[nodename] = node_task_execution_wo(
                        nodename, node["type"], node["nodeobj"], _input, shared_variables, importlibs, test, databases or [])
...
@task(task_run_name="execute-nodes-taskrun-{nodename}",
      log_prints=True)
def execute_node_task(nodename, node_type, node, input, shared_variables, importlibs, test, databases=None):
