/*******************************************************************************
 * BigQuery Standard SQL variant of the achilles_result_concept_count build.
 *
 * Derived from db/migrations/postgres/concept_record_count.sql. Dialect deltas:
 *   - int/bigint -> INT64, varchar(n) -> STRING (BigQuery rejects the postgres
 *     type aliases in DDL and in CAST).
 ...
 *     job and a temp/session table would not survive between them, and a
 *     comment must never contain a separator. The working tables are therefore
 *     permanent scratch tables in ${DATA_CHARACTERIZATION_SCHEMA}, created with
 *     CREATE OR REPLACE TABLE and dropped at the end of the script (same
 *     approach as the hana variant).
 *
...
/************************************************/
DROP TABLE IF EXISTS ${DATA_CHARACTERIZATION_SCHEMA}.achilles_result_concept_count;
...
/**********************************************/
CREATE OR REPLACE TABLE ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts
AS
WITH counts  AS (
...
FROM
counts;
CREATE OR REPLACE TABLE ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts_person
AS
...
FROM
counts_person;
CREATE OR REPLACE TABLE ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_concepts
AS
WITH concepts  AS (
  select concept_id as ancestor_id, coalesce(cast(ca.descendant_concept_id as STRING), concept_id) as descendant_id
  from (
    select concept_id from ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts
    UNION DISTINCT
    ...
    from ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts c
...
FROM
concepts;
INSERT INTO ${DATA_CHARACTERIZATION_SCHEMA}.achilles_result_concept_count (concept_id, record_count, descendant_record_count, person_count, descendant_person_count)
SELECT DISTINCT
...
FROM ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_concepts concepts
         LEFT JOIN ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts c1 ON concepts.ancestor_id = c1.concept_id
         LEFT JOIN ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts c2 ON concepts.descendant_id = c2.concept_id
         LEFT JOIN ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts_person c3 ON concepts.ancestor_id = c3.concept_id
         LEFT JOIN ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts_person c4 ON concepts.descendant_id = c4.concept_id
GROUP BY concepts.ancestor_id;
DROP TABLE IF EXISTS ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_concepts;
DROP TABLE IF EXISTS ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts_person;
DROP TABLE IF EXISTS ${DATA_CHARACTERIZATION_SCHEMA}.dc_tmp_counts;
