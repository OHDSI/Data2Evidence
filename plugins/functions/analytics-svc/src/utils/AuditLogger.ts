...
type AuditCallbackResult = QueryObjectResult | "auditlog disabled" | null;
type AuditCallback = (err: unknown, result?: AuditCallbackResult) => void;
  ...
  private writeLog(
    ...
    const attributesToLog =
      !selectedAttributes || selectedAttributes.length === 0
        ...
        : selectedAttributes;
    const dataAccessMessage = (this.auditLog as {
      read(ref: AuditPatientRef): AuditDataAccessMessage;
    })
    ...
    logAttributes.forEach((logAttribute) => {
      if (logAttribute) {
        try {
          const logMsg =
            `${logAttribute.id} (Configuration: ${this.cdmConfigMetaData.id}, Version: ${this.cdmConfigMetaData.version})`;
          ...
          emptyResult.messageKey = "MRI_PA_CHART_NO_DATA_DEFAULT_MESSAGE";
