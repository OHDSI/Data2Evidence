...
function logAsync(
...
): Promise<{ err: unknown; result: unknown; callbackCount: number }> {
  let callbackCount = 0;
  ...
  return new Promise((resolve) => {
    logger.log(
      ...
      (err, result) => {
        callbackCount += 1;
        resolve({ err, result, callbackCount });
      },
      undefined,
      selectedAttributes,
    );
...
Deno.test(
  ...
  async () => {
    await withMutedConsole(async () => {
      ...
      const callback = () => {};
      const result = await logger.writeFineGrained(
        "pid",
        "patient list",
        [
          {
...
Deno.test(
  "AuditLogger log callback resolves once after all chunks complete",
  async () => {
    await withMutedConsole(async () => {
      env.IS_AUDIT_LOG_ENABLED = "true";
      const { auditLog, messages } = createAuditLogMock();
      ...
      const callbackResult = await logAsync(logger, patients, [
        { id: "pid" },
        { id: "age" },
      ]);
      ...
      assert.equal(callbackResult.callbackCount, 1);
...
Deno.test(
  ...
  async () => {
    await withMutedConsole(async () => {
      ...
      const callbackResult = await logAsync(logger, [
        {
          pid: "patient-1",
          age: 42,
        },
      ...
      assert.equal(callbackResult.err, null);
      assert.equal(callbackResult.result, "auditlog disabled");
      assert.equal(callbackResult.callbackCount, 1);
    });
  },
);
