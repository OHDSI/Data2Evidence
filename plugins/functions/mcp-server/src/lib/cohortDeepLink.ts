...
/**
 * The PA cohort deep link: the bookmark the loader accepts, and the base64url
 * codec that carries it in the URL.
 *
 * Both halves live together because they are one contract — the codec output IS
 * the deep link `query` param, so a change to either side breaks the link.
 */
