...
export interface ConceptSetListItem {
  /**
   * Whatever the model must pass back to address this set. A bare numeric id on
   * the legacy path, a compound `legacy:1` / `webapi:2` ref on d2e-webapi — the
   * listing renders it verbatim either way, so it must be the value the
   * follow-up tool actually accepts.
   */
  id: number | string;
  name: string;
  shared?: boolean | null;
