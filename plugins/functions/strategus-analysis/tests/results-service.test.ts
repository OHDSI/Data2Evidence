...
Deno.test("replaceResult overwrites in place and keeps the id when the filename is unchanged", async () => {
  const storage = fakeStorage();
  const service = serviceWith({
    findOne: () =>
      Promise.resolve({
  ...
  const updated = await service.replaceResult("Bearer test-token", "r1", {
    fileName: "results.zip",
    buffer: ZIP,
    mimetype: "application/zip",
  });

  assertEquals(updated?.id, "r1");
  assertEquals(updated?.name, "Run A");
  assertEquals(updated?.fileSize, 4);
  assertEquals(updated?.checksum, ZIP_SHA256);
  assertEquals(updated?.metadata, { source: "manual" });
  assertEquals(storage.calls.map((c) => c.method), ["upload"]);
});

Deno.test("replaceResult deletes the old object when the filename changes", async () => {
  const storage = fakeStorage();
  const service = serviceWith({
    findOne: () =>
      Promise.resolve({
  ...
  assertEquals(updated?.storagePath, "r1/new.zip");
  assertEquals(storage.calls, [
    { method: "upload", bucket: "strategus-results-store", path: "r1/new.zip" },
    { method: "delete", bucket: "strategus-results-store", path: "r1/old.zip" },
  ]);
});

Deno.test("replaceResult returns null for an unknown id", async () => {
  const service = serviceWith(
    { findOne: () => Promise.resolve(null) },
    fakeStorage(),
  );
  const updated = await service.replaceResult("Bearer test-token", "missing", {
    fileName: "results.zip",
    buffer: ZIP,
    mimetype: "application/zip",
  });
