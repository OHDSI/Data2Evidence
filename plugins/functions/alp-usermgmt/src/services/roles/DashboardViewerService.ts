...
@Service()
export class DashboardViewerService implements IRoleService {
  ...
  async getUsers(): Promise<User[]> {
    const subquery = this.db('user_group')
      ...
      .where('b2c_group.role', DashboardViewerService.ROLE)
  ...
  async getGroup() {
    return await this.groupService.getGroupByRole(DashboardViewerService.ROLE)
  }
  ...
  async isGranted(userId: string): Promise<boolean> {
    ...
    return groups.some(g => g.role === DashboardViewerService.ROLE)
  }

  async registerUser(userId: string) {
    const group = await this.groupService.getGroupByRole(DashboardViewerService.ROLE)
    if (!group?.id) {
      this.logger.warn(`Unable to find group for role ${DashboardViewerService.ROLE}`)
      throw new Error(`Unable to find group for role ${DashboardViewerService.ROLE}`)
    }

    this.logger.info(`Register ${DashboardViewerService.ROLE} for ${userId}`)
  ...
  async withdrawUser(userId: string) {
    ...
    this.logger.info(`Withdraw ${DashboardViewerService.ROLE} for ${userId}`)
