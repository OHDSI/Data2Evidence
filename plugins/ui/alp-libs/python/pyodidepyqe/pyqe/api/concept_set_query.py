...
class ConceptSetQuery(_AuthApi):
    ...
    async def get_all_concept_sets(self) -> List[ConceptSet]:
        ...
        response = await self._get(f"/d2e-webapi/conceptset", headers=headers)
    ...
    async def get_concepts_in_concept_set(
        ...
        Args:
            concept_set_id: ID of a legacy concept set
        """
        return await self.get_concepts_in_concept_set_by_ref(f"legacy:{concept_set_id}")

    async def get_concepts_in_concept_set_by_ref(
        self, ref: str
    ) -> list[ConceptSetConcept]:
        """
