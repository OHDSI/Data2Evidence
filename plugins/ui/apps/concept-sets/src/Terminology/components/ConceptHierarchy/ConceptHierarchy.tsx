...
const numParentLevelsArray = [...Array(numParentLevels).keys()].map(
  (i) => i + 1
);
...
const createNodes = (nodes: ConceptHierarchyNode[]): Node[] => {
  ...
  const nodeLevels: ConceptHierarchyNodeCounts =
    countAndGroupNodesByLevel(nodes);
...
const ConceptHierarchy: FC<ConceptHierarchyProps> = ({
  userId,
  conceptId,
  datasetId,
}) => {
      ...
      const fetchData = async () => {
        if (userId) {
          try {
            ...
            const response = await terminologyApi.getConceptHierarchy(
              datasetId,
              conceptId,
              parseInt(level)
            );
