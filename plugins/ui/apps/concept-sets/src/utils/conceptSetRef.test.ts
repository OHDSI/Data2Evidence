...
describe("CONCEPT_SET_COMPOUND_PATTERN", () => {
  it("matches canonical compound source", () => {
    expect(CONCEPT_SET_COMPOUND_PATTERN.source).toBe(
      "^(legacy|webapi):(0|[1-9]\\d*)$",
    );
  });
});
...
describe("formatConceptSetRef", () => {
  ...
  it("round-trip preserves legacy compound form", () => {
    expect(formatConceptSetRef(parseConceptSetRef("legacy:1"))).toBe(
      "legacy:1",
    );
  });

  it("round-trip preserves webapi compound form", () => {
    expect(formatConceptSetRef(parseConceptSetRef("webapi:42"))).toBe(
      "webapi:42",
    );
  });
});
