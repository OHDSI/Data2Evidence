...
export const ImportDialog: FC<ImportDialogProps> = ({ open, onClose, selectedDatasetId }) => {
  ...
  const handleImport = useCallback(() => {
    const uncheckedData = {
      ...conceptMappingState.importData,
      data: conceptMappingState.importData.data.map((d) => ({ ...d, status: "unchecked" })),
    };

    dispatch({ type: ACTION_TYPES.SET_COLUMN_MAPPING, payload: columnMappingState });
    dispatch({ type: ACTION_TYPES.SET_INITAL_DATA, payload: uncheckedData });
    dispatch({ type: ACTION_TYPES.CLEAR_IMPORT_DATA });
            ...
            <Table stickyHeader>
              ...
              <TableBody>
                {currentPageData.map((dataRow: any, index: React.Key) => (
                  ...
                  >
                    {Object.values(dataRow).map((data: any, index: number) => (
                      <TableCell key={index}>{data}</TableCell>
                    ))}
                  </TableRow>
                ))}
