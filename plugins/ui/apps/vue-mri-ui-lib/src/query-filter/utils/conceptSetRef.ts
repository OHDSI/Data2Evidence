// Mirror of the backend conceptSetRef parser.
// SOURCES OF TRUTH:
//   1. plugins/functions/d2e-webapi/src/utils/conceptSetRef.ts (backend, canonical)
//   2. plugins/ui/apps/concept-sets/src/utils/conceptSetRef.ts (concept-sets frontend mirror)
//
// This is the THIRD mirror (vue-mri-ui-lib query-filter subsystem). Keep all
// three modules in lockstep (behaviour, error messages, regex sources). The
// Zod schemas in the backend are intentionally NOT mirrored here; this
// frontend mirror exposes the runtime parser/formatter/predicate surface only.
...
export const formatConceptSetRef = (ref: ConceptSetRef): string => `${ref.source}:${ref.externalId}`
