    ...
    def task(self, nodes, results, task_run_context):
        with ro.default_converter.context():
            try:
                ...
                dbdao = DBDao(dialect=SupportedDatabaseDialects.TREX if USE_TREX_CONNECTION else None,
                              use_cache_db=False,
...
def construct_jdbc_url(db_credentials):
    conn_url = f'{getattr(DialectDrivers.jdbc, db_credentials.dialect)}://{db_credentials.host}:{db_credentials.port}/{db_credentials.databaseName}'
    if USE_TREX_CONNECTION:
        conn_url += "?preferQueryMode=simple&autocommit=true"
    return conn_url
