    ...
    match flow_action_type:
        ...
        case FlowActionType.INGEST_METADATA:
            ...
            mapping_dbdao = DBDao(use_cache_db=use_cache_db, database_code=database_code,
                                  schema_name=person_patient_mapping.schema_name)
            ...
            if all(schema_exists is False for schema_exists in [mi_schema_exists, vocab_schema_exists, cdm_schema_exists]):
                raise Exception(
                    f"Flow failed! Please ensure all schemas exist before running flow [{mi_dbdao.schema_name}, {vocab_dbdao.schema_name}, {cdm_dbdao.schema_name}]")
            ...
            mapped_concepts_df = get_concept_ids_for_tags(
                extracted_data_df, vocab_dbdao)

            # Assume person record already exists in person table
            new_image_occurrence_id = mi_dbdao.get_next_record_id(
                "image_occurrence", "image_occurrence_id")
            new_procedure_occurrence_id = cdm_dbdao.get_next_record_id(
                "procedure_occurrence", "procedure_occurrence_id")
            new_visit_occurrence_id = cdm_dbdao.get_next_record_id(
            ...
            new_image_feature_id = mi_dbdao.get_next_record_id(
                "image_feature", "image_feature_id")
            new_measurement_id = cdm_dbdao.get_next_record_id(
