...
@task(log_prints=True)
def transform_for_image_feature(mapped_concepts_df: pd.DataFrame,
    ...
    logger.info(grouped__excluded_list.sort_values(
    ...
    logger.info(grouped__selected_data_elements.sort_values(
        by="count", ascending=False))
...
def transform_for_image_occurrence(mapped_concepts_df: pd.DataFrame, vocab_dbdao: DBDao, mapping_dbdao: DBDao,
    ...
    task_logger.info(
        "Successfully created records for image occurrence, procedure occurrence and visit occurrence tables!")
...
def get_concept_ids_for_tags(extracted_data_df: pd.DataFrame, vocab_dbdao: DBDao) -> pd.DataFrame:
    ...
    dicom_tags_omop_df = extracted_data_df.merge(
        concept_df[['concept_id', 'concept_code', 'concept_name']], how='left', left_on='tag', right_on='concept_code')

    # update concept_id data type to int
    dicom_tags_omop_df['concept_id'] = dicom_tags_omop_df['concept_id'].astype(
        'Int64')
...
def extract_data_elements(dicom_files) -> pd.DataFrame:
    ...
    for filepath in dicom_files:
        with dcmread(filepath) as dicom_f:
            ...
            series_instance_uid = dicom_f.get(
                "SeriesInstanceUID")  # [0x0020, 0x000E]
            sop_instance_uid = dicom_f.get("SOPInstanceUID")  # [0x0008,0x0018]
            instance_number = dicom_f.get(
                "InstanceNumber", None)  # [0x0020,0x0013]
    ...
    logger.info(
        f"Successfully extracted metadata for {len(extracted_metadata)} data elements")
