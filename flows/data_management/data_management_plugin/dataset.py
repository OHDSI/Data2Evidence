...
def update_datamodel(
    ...
    try:
        ...
        if data_model in OMOP_DATA_MODELS:
            ...
            cdm_source_row_count = schema_dao.get_table_row_count(
                schema_name, "cdm_source"
            )
...
def insert_cdm_version(schema_dao: DBDao, schema_name: str, cdm_version: str):
    ...
    get_run_logger().info(
        f"Inserting cdm version '{cdm_version}' into '{schema_name}.cdm_source' table.."
    )
    ...
    if is_lower_case:
        ...
        schema_dao.insert_values_into_table(schema_name, "CDM_SOURCE", values_to_insert)
    get_run_logger().info(
        f"Successfully inserted cdm version '{cdm_version}' into '{schema_name}.cdm_source' table.."
    )


@task(log_prints=True)
def update_cdm_version(dbdao: DBDao, schema_name, cdm_version: str):
    get_run_logger().info(
        f"Updating cdm version '{cdm_version}' for '{schema_name}.cdm_source' table.."
    )
    dbdao.update_cdm_version(schema_name, cdm_version)
    get_run_logger().info(
        f"Successfully updated cdm version '{cdm_version}' for '{schema_name}.cdm_source' table.."
    )


def create_cdm_schema_tasks(
    database_code: str,
    data_model: str,
    schema_name: str,
    vocab_schema: str,
