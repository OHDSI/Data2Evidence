...
def search_embedding_plugin(options: SearchEmbeddingType):
    ...
    with duckdb.connect(duckdb_file_path) as conn:
        ...
        conn.execute(
            f"CREATE TABLE {schema_name}.gte_embeddings (concept_id int, vec FLOAT[384]);"
        )
        ...
        for i in range(0, length, step):
            ...
            embeddings = embedding_concept_table(
                concept_name, tokenizer, model
            ).tolist()
            rst = pd.DataFrame({"concept_id": concept_id, "gte-small_384": embeddings})
            conn.execute(
                f"""INSERT INTO {schema_name}.gte_embeddings SELECT concept_id, "gte-small_384" FROM rst"""
            )
        ...
        conn.execute(
            f"CREATE INDEX gte_cos_idx ON {schema_name}.concept USING HNSW (concept_name_embedding) WITH (metric = 'cosine')"
        )
