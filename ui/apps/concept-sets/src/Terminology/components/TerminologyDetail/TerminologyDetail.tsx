...
const TerminologyDetail: FC<TerminologyDetailProps> = ({
  ...
  const [tabValue, setTabValue] = useState(
    TerminologyDetailsTab.RelatedConcepts
  );

  const handleTabSelectionChange = useCallback(
    (_event: React.SyntheticEvent, value: TerminologyDetailsTab) => {
      setTabValue(value);
    },
    []
  );
      ...
      const fetchData = async () => {
        if (userId) {
          try {
            ...
            const fhirResponse = await terminologyApi.getTerminologyConnections(
              conceptId,
              datasetId
            );
            const response: TerminologyDetailsList = {
              details:
                fhirResponse.group[0]?.element?.[0]?.valueSet.expansion
                  .contains[0],
              connections: [],
            };
            ...
            setFeedback({
              ...
              description: getText(
                i18nKeys.TERMINOLOGY_DETAIL__ERROR_DESCRIPTION
              ),
            });
          } finally {
            setIsLoading(false);
              ...
              <div className="terminology_detail__table-connections">
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow>
                      <TableCell width="20%">
                        {getText(i18nKeys.TERMINOLOGY_DETAIL__RELATIONSHIP)}
                      </TableCell>
                      <TableCell width="30%">
                        {getText(i18nKeys.TERMINOLOGY_DETAIL__RELATES_TO)}
                      </TableCell>
                      <TableCell width="30%">
                        {getText(i18nKeys.TERMINOLOGY_DETAIL__CONCEPT_ID)}
                      </TableCell>
                      <TableCell width="20%">
                        {getText(i18nKeys.TERMINOLOGY_DETAIL__VOCABULARY)}
                      </TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody
                ...
                {data && data?.connections.length > 0 && (
                  <Table size="small" stickyHeader>
                    ...
                    >
                      ...
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__ID)}
                        </TableCell>
                        <TableCell>{data?.details?.conceptId ?? ""}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__CODE)}
                        </TableCell>
                        <TableCell>{data?.details?.code ?? ""}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__NAME)}
                        </TableCell>
                        <TableCell>{data?.details?.display ?? ""}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__CLASS)}
                        </TableCell>
                        <TableCell>
                          {data?.details?.conceptClassId ?? ""}
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__CONCEPT)}
                        </TableCell>
                        <TableCell>{data?.details?.concept ?? ""}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__DOMAIN)}
                        </TableCell>
                        <TableCell>{data?.details?.domainId ?? ""}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__VOCABULARY)}
                        </TableCell>
                        <TableCell>{data?.details?.system ?? ""}</TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>
                          {getText(i18nKeys.TERMINOLOGY_LIST__VALIDITY)}
                        </TableCell>
