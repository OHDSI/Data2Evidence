...
const ConceptSetDeleteDialog: FC<ConceptSetDeleteDialogProps> = ({
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      let errorMessage = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_OCCURRED);
      let errorDescription = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_OCCURRED_DESCRIPTION);

      if (status === 409 && responseData?.error === "CONCEPT_SET_IN_USE") {
        // Concept set is in use - show detailed message
        errorMessage = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_IN_USE);
        const usageDetails: string[] = [];
        if (responseData.cohortDefinitions?.length > 0) {
          const cohortNames = responseData.cohortDefinitions.map((c: { name: string }) => c.name).join(", ");
          usageDetails.push(`${getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_IN_USE_COHORTS)}: ${cohortNames}`);
        }
        if (responseData.bookmarks?.length > 0) {
          const bookmarkNames = responseData.bookmarks.map((b: { name: string }) => b.name).join(", ");
          usageDetails.push(`${getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_IN_USE_BOOKMARKS)}: ${bookmarkNames}`);
        }
        errorDescription = usageDetails.join("\n");
      } else if (status === 403) {
        errorMessage = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_FORBIDDEN);
        errorDescription = "";
      } else if (status === 404) {
        errorMessage = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_NOT_FOUND);
        errorDescription = "";
      } else if (status >= 500) {
        errorMessage = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_SERVER);
        errorDescription = getText(i18nKeys.CONCEPT_SET_DELETE_DIALOG__ERROR_OCCURRED_DESCRIPTION);
      }
  ...
  }, [conceptSet, datasetId, handleClose, setMainFeedback, onDeleted, getText, i18nKeys]);
