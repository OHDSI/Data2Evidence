...
export const getExtCohortKeyForEvent = (cdmConfig: CdmConfig, path?: string) => {
  if (!path) {
    return null
  }
...
export const getExtCohortInfoForAttribute = (cdmConfig: CdmConfig, path?: string) => {
  if (!path) {
    return null
  }
...
const extractConceptSets = async (filter: IFRFilterCardContent, cdmConfig: CdmConfig, datasetId: string) => {
  ...
  for (
    ...
    for (
      ...
      constraintContentIndex < filter.attributes?.content[attributesContentIndex].constraints?.content.length;
      constraintContentIndex += 1
    ) {
      const constraintContent =
        filter.attributes?.content?.[attributesContentIndex]?.constraints?.content?.[constraintContentIndex]
      const conceptValue = constraintContent && 'value' in constraintContent ? constraintContent.value : undefined

      const cohortDefinitionKey: string | null =
        _.get(cdmConfig, `${filter.attributes?.content?.[attributesContentIndex].configPath}.cohortDefinitionKey`) ||
        null
      const conceptIdentifierType: 'name' | 'code' | 'id' | null =
        _.get(cdmConfig, `${filter.attributes?.content?.[attributesContentIndex].configPath}.conceptIdentifierType`) ||
        null
      const type: 'text' | 'time' | 'datetime' | 'conceptSet' | 'num' | null =
        _.get(cdmConfig, `${filter.attributes?.content?.[attributesContentIndex].configPath}.type`) || null
...
export const convertAttributeContentToCriteria = ({ attrType, content }: Params) => {
  const OPERATORS = {
    '>=': 'gte',
    '>': 'gt',
    '<=': 'lte',
  ...
  switch (attrType) {
    ...
    case 'time':
      ...
      if (content.length === 1) {
        ...
        if (typeof timeContent.value !== 'string' || typeof timeContent1.value !== 'string') {
          return null
        }
        return {
          Op: 'bt',
...
export const createDemographicCriteriaList = async (
  ...
  for (let i = 0; i <= content.length; i += 1) {
    ...
    const attributeInfo = getExtCohortInfoForAttribute(cdmConfig, filterCard.configPath)
    if (!(attributeInfo?.key && attributeInfo?.type)) {
      continue
    }

    const conceptIdentifierType: 'name' | 'code' | 'id' | null =
      _.get(cdmConfig, `${filterCard.configPath}.conceptIdentifierType`) || null
    ...
    if (constraintContent && 'operator' in constraintContent) {
      ...
      } else if (typeof valueOrConceptName === 'number' && attributeInfo.type !== 'time') {
...
export const getCodesetId = (conceptSets: ExtCohortConceptSet[], instanceId: string) => {
...
export const createCriteriaList = async (
  ...
  for (let groupIndex = 0; groupIndex < nonBasicFilterCards.length; groupIndex += 1) {
    const singleOrGroup = nonBasicFilterCards[groupIndex]
    const eventsForFilter: { [key: string]: any[] } = {}
    for (let withinGroupIndex = 0; withinGroupIndex < singleOrGroup.content.length; withinGroupIndex += 1) {
      if (singleOrGroup.content[withinGroupIndex]['op'] === 'NOT') {
        let excludedEventObj: Object = {
          isExcluded: true,
        }
        const excludedCard = singleOrGroup.content[withinGroupIndex] as IFRFilterCard
        for (const c of excludedCard.content) {
          ...
          const eventKey = getExtCohortKeyForEvent(cdmConfig, c.configPath)
          if (!eventKey) {
            continue
          }
      ...
      let eventObject = {}
      const content = singleOrGroup.content[withinGroupIndex] as IFRFilterCardContent
      ...
      for (let attrIndex = 0; attrIndex < content.attributes.content.length; attrIndex += 1) {
        const attribute = content.attributes.content[attrIndex]
        const attributeInfo = getExtCohortInfoForAttribute(cdmConfig, attribute.configPath)
        ...
        if (attributeInfo?.key && attributeInfo?.type) {
          ...
          for (const constraintContent of attribute.constraints.content) {
            if ('op' in constraintContent) {
              for (let i = 0; i < constraintContent.content.length; i += 1) {
                let criteria: ReturnType<typeof convertAttributeContentToCriteria>
                if (attributeInfo.type === 'time') {
                  criteria = convertAttributeContentToCriteria({
                    attrType: attributeInfo.type,
              ...
              if (attributeInfo.key === 'CodesetId') {
                const conceptSet = conceptSets.find(cset => cset.name === attribute.configPath)
                if (!conceptSet) {
                  continue
                }
                attributesForFilter[attributeInfo.key] = conceptSet.id
              } else {
                const conceptIdentifierType: 'name' | 'code' | 'id' | null =
       ...
       * END ATTRIBUTES
       */
      const eventKey = getExtCohortKeyForEvent(cdmConfig, content.configPath)
      if (!eventKey) {
        continue
      }
...
export const createInclusionRules = (demographicCriteriaList: { [key: string]: Criteria }[], criteriaList: any[]) => {
