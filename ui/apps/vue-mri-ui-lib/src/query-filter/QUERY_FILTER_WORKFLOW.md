...
- **QueryFilterEvent**: Represents individual medical events (conditions, drugs, procedures)
...
Transforms UI model back to Atlas JSON structure:

1. **Structure Creation**: Builds Atlas PrimaryCriteria, InclusionRules, CensoringCriteria
2. **Concept Set Mapping**: Maps UI concept sets back to Atlas format
3. **Operator Conversion**: Converts internal operators back to Atlas format
4. **Nested Reconstruction**: Rebuilds CorrelatedCriteria from nested attributes
