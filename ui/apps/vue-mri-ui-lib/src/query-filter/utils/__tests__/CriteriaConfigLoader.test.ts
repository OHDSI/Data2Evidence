...
import type { CohortExpression, CriteriaConfig } from '../CriteriaConfigLoader'
import criteriaConfigData from '../../config/cohort-criteria-config.json'

describe('CriteriaConfigLoader', () => {
  let loader: CriteriaConfigLoader
  let mockExpression: CohortExpression

  beforeEach(() => {
    // Use default config for standard tests
    loader = new CriteriaConfigLoader(criteriaConfigData as CriteriaConfig)
    mockExpression = {
  ...
  const createTestLoader = (configModifier: (config: any) => any) => {
    ...
    return new CriteriaConfigLoader(modifiedConfig)
  }

  describe('constructor and property initialization', () => {
    test('should initialize with config data', () => {
      expect(loader).toBeInstanceOf(CriteriaConfigLoader)
    })
  ...
  describe('getCriteriaOptions', () => {
    test('should return criteria options for initialEvents section', () => {
      ...
      const testLoaderForGroupCheck = createTestLoader(config => {
        ...
        config.criteriaTypes.testGroupOnly = {
          ...
          class: 'TestGroupOnly',
          icon: 'test-icon',
          atlasKey: 'addTestGroupOnly',
          groupOnly: true,
          descriptions: { group: 'Test group only description' },
        }
        config.sections.initialEvents.criteriaTypes.push('testGroupOnly')
        return config
      })
    ...
    test('should return criteria options for censoringEvents section', () => {
      ...
      const testLoaderForGroupCheck = createTestLoader(config => {
        ...
        config.sections.censoringEvents.criteriaTypes.push('testGroupOnly')
        return config
      })
    ...
    test('should return criteria options for criteriaGroup section', () => {
      ...
      const testLoaderForGroupCheck = createTestLoader(config => {
        ...
        config.sections.criteriaGroup.criteriaTypes.push('testGroupOnly')
        return config
      })
  ...
  describe('getAtlasKey', () => {
    ...
    test('should return criteria type id if no atlasKey exists', () => {
      ...
      const testLoader = createTestLoader(config => {
        const modifiedType = { ...config.criteriaTypes.conditionOccurrence }
        delete modifiedType.atlasKey
        config.criteriaTypes.conditionOccurrence = modifiedType
        return config
      })

      const atlasKey = testLoader.getAtlasKey('conditionOccurrence')
      expect(atlasKey).toBe('conditionOccurrence')
    })
